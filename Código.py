...
def inserir_animal():
    ...
    try:
        ...
        if not cursor.fetchone():
            ...
            cursor.execute("INSERT INTO clinica.pessoa VALUES (%s,%s,%s)", (cpf, nome_t, sobrenome))
            ...
            cursor.execute("INSERT INTO clinica.tutor VALUES (%s,%s)", (cpf, data))
        ...
        conex.rollback()
        print(f"\n[ERRO NO BANCO DE DADOS] Não foi possível inserir o animal: {erro}")
    finally:
        cursor.close()
        conex.close()
...
def inserir_veterinario():
    ...
    try:
        ...
        if not cursor.fetchone():
            ...
            cursor.execute("INSERT INTO clinica.pessoa VALUES (%s,%s,%s)", (cpf, nome, sobrenome))
        ...
        if not cursor.fetchone():
            ...
            if salario is None:
                ...
                return
            cursor.execute("INSERT INTO clinica.funcionario VALUES (%s,%s,%s)", (cpf, matricula, salario))
        ...
        cursor.execute("INSERT INTO clinica.veterinario VALUES (%s,%s,%s)", (cpf, crmv, esp))
        ...
        conex.rollback()
        print(f"\n[ERRO NO BANCO DE DADOS] Não foi possível inserir o veterinário: {erro}")
    finally:
        cursor.close()
        conex.close()
...
def inserir_consulta():
    ...
    try:
        ...
        cursor.execute("SELECT 1 FROM clinica.animal WHERE id_chave=%s", (id_animal,))
        if not cursor.fetchone():
        ...
        cpf = int(cpf)
        cursor.execute("SELECT 1 FROM clinica.veterinario WHERE funcionario_cpf=%s", (cpf,))
        if not cursor.fetchone():
        ...
        conex.rollback()
        print(f"\n[ERRO NO BANCO DE DADOS] Não foi possível inserir a consulta: {erro}")
    finally:
        cursor.close()
        conex.close()

# ==========================================
# FUNÇÕES DE DELETAR (DELETE)
# ==========================================
def deletar_animal():
    conex = conexao()
    cursor = conex.cursor()
...
def atualizar_animal():
    ...
    try:
        ...
        id_chave = input("ID animal: ")
        cursor.execute("SELECT * FROM clinica.animal WHERE id_chave=%s", (id_chave,))
        ...
        if op == "1":
            ...
            cursor.execute("UPDATE clinica.animal SET nome=%s WHERE id_chave=%s", (nome, id_chave))
        elif op == "2":
            especie = input("Nova espécie: ")
            cursor.execute("UPDATE clinica.animal SET especie=%s WHERE id_chave=%s", (especie, id_chave))
        elif op == "3":
            raca = input("Nova raça: ")
            cursor.execute("UPDATE clinica.animal SET raca=%s WHERE id_chave=%s", (raca, id_chave))
        elif op == "4":
            ...
            if not cursor.fetchone():
                print("Tutor não existe.")
                return
            cursor.execute("UPDATE clinica.animal SET tutor_cpf=%s WHERE id_chave=%s", (cpf, id_chave))
        elif op == "5":
            ...
            if not data:
                print("Data inválida.")
                return
            cursor.execute("UPDATE clinica.animal SET data_nascimento=%s WHERE id_chave=%s", (data, id_chave))
        elif op == "6":
            idade = validar_int(input("Nova idade: "))
            if idade is None or idade < 0:
                print("Idade inválida.")
                return
            cursor.execute("UPDATE clinica.animal SET idade=%s WHERE id_chave=%s", (idade, id_chave))
        else:
        ...
        conex.rollback()
        print(f"\n[ERRO NO BANCO DE DADOS] Não foi possível atualizar: {erro}")
    finally:
        cursor.close()
        conex.close()
...
def atualizar_veterinario():
    ...
    try:
        ...
        cpf = int(cpf)
        cursor.execute("SELECT * FROM clinica.veterinario WHERE funcionario_cpf=%s", (cpf,))
        if not cursor.fetchone():
            print("Veterinário não existe.")
            return
        ...
        if op == "1":
            ...
            cursor.execute("UPDATE clinica.veterinario SET crmv=%s WHERE funcionario_cpf=%s", (crmv, cpf))
        elif op == "2":
            esp = input("Nova especialidade: ")
            cursor.execute("UPDATE clinica.veterinario SET especialidade=%s WHERE funcionario_cpf=%s", (esp, cpf))
        else:
            print("Opção inválida.")
            return
...
def atualizar_consulta():
    ...
    try:
        ...
        cursor.execute("SELECT * FROM clinica.consulta WHERE id_consulta=%s", (id,))
        if not cursor.fetchone():
        ...
        if op == "1":
            ...
            cursor.execute("UPDATE clinica.consulta SET diagnostico=%s WHERE id_consulta=%s", (d, id))
        elif op == "2":
            ...
            if not h:
                print("Hora inválida.")
                return
            cursor.execute("UPDATE clinica.consulta SET horario=%s WHERE id_consulta=%s", (h, id))
        elif op == "3":
            print("Formato data: YYYY-MM-DD")
            data = validar_data(input("Nova data: "))
            if not data:
                print("Data inválida.")
                return
            cursor.execute("UPDATE clinica.consulta SET data_consulta=%s WHERE id_consulta=%s", (data, id))
        else:
            print("Opção inválida.")
            return
...
while True:
    ...
    if op == "1":
        if x == "1": inserir_animal()
        elif x == "2": inserir_veterinario()
        elif x == "3": inserir_consulta()
    elif op == "2":
        if x == "1": listar_animais()
        elif x == "2": listar_veterinarios()
        elif x == "3": listar_consultas()
    elif op == "3":
        if x == "1": atualizar_animal()
        elif x == "2": atualizar_veterinario()
        elif x == "3": atualizar_consulta()
    elif op == "4":
        if x == "1": deletar_animal()
        elif x == "2": deletar_veterinario()
        elif x == "3": deletar_consulta()
